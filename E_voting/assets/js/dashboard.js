...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentUser.voterType === 'student' || currentUser.type === 'voter') {
        // Use the new combined initialization function for student dashboard
        initializeStudentDashboard();
    } else if (currentUser.voterType === 'staff' || currentUser.type === 'admin') {
        // Load elections and results for staff/admin
    ...
    function initializeTabs() {
        ...
        // Check if we should show the voted tab (coming from vote.js after voting)
        const showVotedTab = sessionStorage.getItem('showVotedTab') === 'true';
        if (showVotedTab) {
            // Clear the instruction
            sessionStorage.removeItem('showVotedTab');
            
            // Find and click the voted tab
            const votedTab = document.querySelector('.dashboard-tab[data-tab="voted"]');
            if (votedTab) {
                ...
                // Add active class to voted tab and section
                votedTab.classList.add('active');
                const votedSection = document.getElementById('voted');
                if (votedSection) {
                    votedSection.classList.add('active');
                    votedSection.style.display = 'block';
                }
                
                // Load voted elections data
                loadVotedElections();
            }
        }
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                if (targetId === 'results' && currentUser.voterType === 'student') {
                    ...
                    loadAllResults();
                } else if (targetId === 'voted') {
                    // Refresh voted elections when switching to that tab
                    loadVotedElections();
                }
            });
        });
    ...
    function loadStudentElections() {
        ...
        const eligibleElections = allElections.filter(election => {
            ...
            // Always exclude elections where user has already voted
            if (hasVoted) {
                return false;
            }
            ...
            if (election.eligibility === 'authorized') {
                // For authorized elections, check if the user's ID is in the authorizedUsers array
                const isAuthorized = election.authorizedUsers && 
                    (election.authorizedUsers.includes(currentUser.id) || 
                     (currentUser.studentId && election.authorizedUsers.includes(currentUser.studentId)) ||
                     (currentUser.identificationNumber && election.authorizedUsers.includes(currentUser.identificationNumber)));
        ...
        if (eligibleElections.length === 0) {
            ...
            electionsContainer.innerHTML = ''; // Clear any existing content
        ...
        eligibleElections.forEach(election => {
                ...
                <div class="election-body">
                    ...
                    <div class="election-actions">
                        ${election.status === 'active' 
                            ? `<a href="vote-verification.html" class="btn btn-primary vote-btn" data-id="${election.id}"><i class="fas fa-vote-yea"></i> Vote Now</a>
            ...
            // Add event listener for the vote button - Redirect to QR verification first
            const voteBtn = electionCard.querySelector('.vote-btn');
            if (voteBtn) {
                voteBtn.addEventListener('click', () => {
                    // Store election ID in session storage for verification page
                    sessionStorage.setItem('selectedElectionId', election.id);
                    // Redirect to vote verification page
    ...
    function loadStudentResults() {
        ...
        resultElections.forEach(election => {
                ...
                <div class="result-header">
                    ...
                    ${!hasVoted && election.status === 'active' ? 
                      `<div class="not-voted-warning">
                          ...
                          <a href="vote-verification.html" class="btn btn-sm btn-primary" data-id="${election.id}">Vote Now</a>
                       </div>` : ''}
                </div>
            ...
            // Add event listener for Vote Now links in results
            const voteLink = resultCard.querySelector('.not-voted-warning a');
            if (voteLink) {
                voteLink.addEventListener('click', (e) => {
                    e.preventDefault();
                    // Store election ID in session storage for verification
                    sessionStorage.setItem('selectedElectionId', election.id);
                    // Redirect to vote verification page
                    window.location.href = 'vote-verification.html';
                });
    ...
    function initializeConductElectionForm() {
        ...
        function handleCSVUpload(event) {
            ...
            reader.onload = function(e) {
                ...
                // Log the parsed authorized users
                console.log("Parsed authorized users from CSV:", authorizedUsers);
        ...
        if (createElectionForm) {
            createElectionForm.addEventListener('submit', (e) => {
                ...
                const newElection = {
                    ...
                    candidates,
                    voters: [],
                    authorizedUsers: eligibility === 'authorized' ? 
                        authorizedUsers.map(user => user.trim()).filter(user => user !== '') : [],
                    settings: {
                        anonymousVoting,
                        publicResults,
                        realTimeResults
                ...
                console.log('Creating new election:', newElection);
    ...
    // Function to load and display elections that the user has voted in
    function loadVotedElections() {
        const allElections = JSON.parse(localStorage.getItem('elections')) || [];
        const currentUser = JSON.parse(sessionStorage.getItem('currentUser'));
        ...
        console.log("Checking voted elections for user:", currentUser.id);
        
        // Filter elections where the user has voted
        const votedElections = allElections.filter(election => {
            const hasVoted = election.voters && election.voters.includes(currentUser.id);
            
            // Debug specific elections (especially CR Election)
            if (election.title && election.title.toLowerCase().includes('cr')) {
                console.log(`CR Election "${election.title}" voted check:`, {
        ...
        console.log("Found voted elections:", votedElections.map(e => e.title));
        
        const votedElectionsContainer = document.getElementById('votedElectionsContainer');
        if (!votedElectionsContainer) return;
        
        // Check if there are any voted elections
        if (votedElections.length === 0) {
            document.getElementById('noVotedElectionsMessage').style.display = 'block';
            votedElectionsContainer.innerHTML = ''; // Clear any previous content
            const emptyState = document.getElementById('noVotedElectionsMessage');
            if (emptyState) {
                votedElectionsContainer.appendChild(emptyState);
            }
            return;
        }
        ...
        votedElections.forEach(election => {
            const electionCard = document.createElement('div');
            electionCard.className = 'election-card voted';
            ...
            electionCard.innerHTML = `
                ...
                <div class="election-body">
                    <p class="election-description">${election.description}</p>
                    <div class="election-actions">
                        ${(election.settings.publicResults || election.settings.realTimeResults) ? 
                            `<a href="vote.html?id=${election.id}" class="btn btn-secondary"><i class="fas fa-chart-bar"></i> View Results</a>` : 
                            `<button class="btn btn-primary" disabled><i class="fas fa-check-circle"></i> Vote Recorded</button>`}
                    </div>
                </div>
            `;
            
            // Add event listener for the view results button
            const viewResultsBtn = electionCard.querySelector('a.btn-secondary');
            if (viewResultsBtn) {
            ...
            votedElectionsContainer.appendChild(electionCard);
        });
    }
}); 